...
class Testing(unittest.TestCase):
    ...
    def which_method_to_test(self, which, x) -> int:
        ...
        if which == 1:
            z = Solution().copy_random_list(x)
            return z
        elif which == 2:
            z = Solution2().copy_random_list_2(x)
            return z
        elif which == 3:
            z = Solution3().copy_random_list_3(x)
            return z
    ...
    def test_1(self):
        ...
        test_description = which_test[1]
        for w in range(1, METHOD_COUNT + 1):
            result = self.which_method_to_test(w, test_input)
            is_copy = Solution().confirm_as_deepcopy(result, test_input)
            self.assertTrue(
                is_copy,
